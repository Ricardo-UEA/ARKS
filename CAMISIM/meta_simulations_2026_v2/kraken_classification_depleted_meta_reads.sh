...
#SBATCH -o job_outputs/kraken_depletion_all_%j.out
#SBATCH -e job_errors/kraken_depletion_all_%j.err
#SBATCH -p hmem
#SBATCH --qos=hmem
#SBATCH --mem=700GB
...
READS_DIR=${BASE_DIR}/depleted_reads
REPORTS_DIR=${BASE_DIR}/kraken_depleted_reports_hdist_0_mcf_05
OUTPUTS_DIR=${BASE_DIR}/kraken_depleted_outputs_hdist_0_mcf_05
...
echo ""
# Track progress
TOTAL_SAMPLES=$(ls -1 ${READS_DIR}/meta_sample_*_depleted_R1.fastq.gz 2>/dev/null | wc -l)
CURRENT=0
FAILED=0
echo "Found $TOTAL_SAMPLES samples to process"
echo ""
# Loop through all samples
for R1 in ${READS_DIR}/meta_sample_*_depleted_R1.fastq.gz; do
    [[ -e "$R1" ]] || continue
    
    # Extract sample name
    SAMPLE=$(basename "$R1" _depleted_R1.fastq.gz)
    R2="${READS_DIR}/${SAMPLE}_depleted_R2.fastq.gz"
    ...
    if [[ $? -ne 0 ]]; then
        echo "ERROR: KrakenUniq failed on $SAMPLE"
        FAILED=$((FAILED + 1))
        continue
    fi
