...
Host-depleted reads only (kraken_depleted_reports).
...
Usage:
    python evaluate_simulations_depleted.py /gpfs/home/hce24xau/scratch/gen_kmers/data/simulations/CAMISIM/meta_simulations_2026_v2 --samples 100 --output results/depleted

HPC Directory structure:
    base_dir/
    ├── kraken_depleted_reports/meta_sample_1.report ... meta_sample_N.report
    └── output_files/meta_sample_1/taxonomic_profile_0.txt ... meta_sample_N/
"""
...
def find_files(base_dir, sample_num):
    """Find truth and report files for a sample (depleted reads only)."""
    ...
    report_candidates = [
        base_dir / "kraken_depleted_reports" / f"{sample_name}.report",
    ]
...
def main():
    ...
    print(f"Read type: host-depleted")
