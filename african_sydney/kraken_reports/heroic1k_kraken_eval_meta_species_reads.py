#!/usr/bin/env python3
"""
HEROIC1K KrakenUniq Report Evaluation Pipeline — Species Level, READS, with Metadata
======================================================================================
Parses KrakenUniq reports at SPECIES rank and integrates GEOETHNIC_MASTER
metadata for ancestry-aware, clinically-contextualised analysis.

** READ-COUNT VERSION **
This script uses reads_clade (clade-level read counts) instead of kmer counts
as the primary abundance metric. All matrices, diversity indices, plots, and
summary statistics are computed from read counts rather than kmers.

  reads_clade  = reads assigned to this taxon + all descendants (col 2 in report)
  reads_direct = reads assigned directly to this taxon only     (col 3 in report)

By default reads_clade is used (consistent with the kmer version's clade-level
aggregation). Pass --use_direct_reads to switch to reads_direct instead.

Usage:

#Non depletion usage
python heroic1k_kraken_eval_meta_species_reads.py \
  ...
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_no_depletion/meta_data_analysis_species_reads \
  --cov_threshold 0.005 \
  --top_species 40


#GRCh38 depletion Evaluation
python heroic1k_kraken_eval_meta_species_reads.py \
  --report_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/GRCh38_mcf_05_depletion_reports \
  --metadata /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/geoethnic_master_batch1.csv \
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_depletion_GRCh38_mcf_05_species_reads \
  --cov_threshold 0.005 \
  --top_species 40

#Panhuman depletion usage
python heroic1k_kraken_eval_meta_species_reads.py \
  --report_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/panhuman_mcf_05_depletion_reports \
  --metadata /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/geoethnic_master_batch1.csv \
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_depletion_panhuman_species_reads \
  --cov_threshold 0.005 \
  --top_species 40

#Panhuman depletion usage
python heroic1k_kraken_eval_meta_species_reads_1+2.py \
  --report_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/panhuman_mcf_05_depletion_reports \
  --metadata /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/geoethnic_master_batch_1-2.csv \
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_depletion_panhuman_species_reads \
  --cov_threshold 0.005 \
  --top_species 40
...
python heroic1k_kraken_eval_meta_species_reads.py \
  ...
  --metadata /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/geoethnic_master_batch1.csv \
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_depletion_t2t_species_reads \
  --cov_threshold 0.005 \
  --top_species 40

#Use direct reads instead of clade reads
python heroic1k_kraken_eval_meta_species_reads.py \
...
def parse_report(filepath):
    ...
    return species, unclass_reads + root_reads, human_reads, unclass_reads, root_reads


def summarise_sample(sid, species, total_reads, human_reads, unclass_reads, classified):
    ...
    return {
        ...
        "n_species_detected":    len(species),
    }
...
def load_all(report_dir, pattern, cov_threshold, use_direct_reads=False):
    read_field = "reads_direct" if use_direct_reads else "reads_clade"
    read_label = "Direct reads" if use_direct_reads else "Clade reads"
    print(f"[INFO] Abundance metric: {read_label} ({read_field})")
    ...
    diversity    = []
    flagged      = []
    read_matrix  = defaultdict(dict)

    for f in files:
        sid = f.name.split(".krakenuniq")[0].split(".report")[0]
        species, total, human, unclass, classified = parse_report(f)
        summaries.append(summarise_sample(sid, species, total, human, unclass, classified))

        # Diversity computed from read counts, not kmers
        read_counts = [s[read_field] for s in species if s[read_field] > 0]
        diversity.append({
            "sample_id": sid, "observed_species": len(species),
            "shannon": shannon(read_counts), "simpson": simpson(read_counts),
        })

        for s in species:
            read_matrix[s["name"]][sid] = s[read_field]
            if s["cov"] >= cov_threshold:
                flagged.append({
                    "sample_id": sid, "species": s["name"],
                    "reads_clade":  s["reads_clade"],
                    "reads_direct": s["reads_direct"],
                    "reads_used":   s[read_field],
                    "kmers": s["kmers"],
                    "dup": s["dup"], "cov": s["cov"],
                    "is_contaminant": any(c.lower() in s["name"].lower() for c in CONTAMINANTS),
                })

    df_summ = pd.DataFrame(summaries)
    df_div  = pd.DataFrame(diversity)
    df_flag = pd.DataFrame(flagged) if flagged else pd.DataFrame(
        columns=["sample_id","species","reads_clade","reads_direct","reads_used",
                 "kmers","dup","cov","is_contaminant"])

    all_samples = [s["sample_id"] for s in summaries]
    df_reads = pd.DataFrame(read_matrix, index=all_samples).fillna(0).astype(int)

    return df_summ, df_div, df_flag, df_reads
...
def plot_alpha_diversity(df, out_dir):
    ...
    fig.suptitle("HEROIC1K — Species-Level Alpha Diversity", fontsize=12, fontweight="bold")
    ...
    for ax, col, label, color in zip(
        ...
        ["Observed Species", "Shannon Entropy (reads)", "Simpson Index (reads)"],
...
def plot_ancestry_diversity(df, out_dir):
    """Shannon and observed species by ancestry — boxplots + strip."""
    anc_order = ["Southern African", "European", "Admixed", "Asian", "Unknown"]
    anc_order = [a for a in anc_order if a in df["ancestry"].values]
    ...
    for ax, col, label in zip(
        axes,
        ["shannon", "observed_species"],
        ["Shannon Entropy", "Observed Species"],
    ):
        data_by_anc = [df.loc[df["ancestry"] == a, col].dropna().values for a in anc_order]
...
def plot_flagged_heatmap(df_flag, df_meta_summ, out_dir, cov_threshold):
    if df_flag.empty:
        print(f"[INFO] No species flagged at coverage >= {cov_threshold}"); return

    pivot = df_flag.pivot_table(
        index="sample_id", columns="species", values="cov", aggfunc="max", fill_value=0
    )
    ...
    ax.set_title(f"High-Confidence Species (coverage ≥ {cov_threshold}) — Coverage per Sample",
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "07_flagged_species_heatmap.png", dpi=150, bbox_inches="tight")
    plt.close(); print("[PLOT] 07_flagged_species_heatmap.png")


def plot_read_matrix(df_reads, df_meta_summ, out_dir, top_n=40):
    if df_reads.empty: return

    human_like = [c for c in df_reads.columns
                  if any(h.lower() in c.lower() for h in ["homo", "human"])]
    df_filt = df_reads.drop(columns=human_like, errors="ignore")
    top_species = df_filt.sum(axis=0).nlargest(top_n).index
    df_top = df_filt[top_species]
    ...
    sns.heatmap(df_log, cmap="Blues", linewidths=0, ax=ax,
                cbar_kws={"label": "log₁₀(reads + 1)", "shrink": 0.6}, yticklabels=False)
    ax.set_title(f"Species Read Abundance Matrix — Top {top_n} Species (log₁₀, Bray-Curtis clustered)",
                 fontsize=11, fontweight="bold")
    ax.set_xlabel("Species"); ax.set_ylabel(f"Samples (n={len(df_log)})")
    plt.xticks(rotation=45, ha="right", fontsize=8)
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "08_read_matrix_heatmap.png", dpi=150, bbox_inches="tight")
    plt.close(); print("[PLOT] 08_read_matrix_heatmap.png")


def plot_flagged_prevalence(df_flag, out_dir, cov_threshold):
    if df_flag.empty: return
    prev = (df_flag.groupby("species")["sample_id"].nunique()
            .sort_values(ascending=False).head(25).reset_index())
    prev.columns = ["species", "n_samples"]
    colors = [
        "#FF6F00" if any(c.lower() in g.lower() for c in CONTAMINANTS) else "#0288D1"
        for g in prev["species"]
    ]
    fig, ax = plt.subplots(figsize=(10, max(5, len(prev) * 0.35)))
    ax.barh(prev["species"][::-1], prev["n_samples"][::-1], color=colors[::-1])
    ax.set_xlabel("Number of Samples")
    ax.set_title(f"Top 25 Flagged Species by Prevalence (coverage ≥ {cov_threshold})",
                 fontsize=11, fontweight="bold")
    ax.legend(handles=[mpatches.Patch(facecolor="#0288D1", label="Microbial species"),
...
def write_report(df, df_div, df_flag, df_reads, cov_threshold, out_dir):
    lines = []
    sep = "=" * 68

    lines += [sep, "HEROIC1K KrakenUniq Evaluation — Summary Report (Species Level, READS)", sep, ""]
    lines.append(f"  Samples analysed:         {len(df)}")
    lines.append(f"  Coverage flag threshold:   >= {cov_threshold}")
    lines.append(f"  Total species detected:    {df_reads.shape[1]}")
    lines.append("")
    ...
    lines.append("── Alpha Diversity (Species Level, Reads) " + "─" * 19)
    lines.append(f"  Median observed species:  {df_div['observed_species'].median():>8.0f}")
    ...
    lines.append(f"  Spearman r (depth~shannon): {r:.3f}  p={p:.3e}  [read-based diversity]")
    lines.append("")

    lines.append(f"── Flagged Species (coverage >= {cov_threshold}) " + "─" * 24)
    if df_flag.empty:
        lines.append("  None detected.")
    else:
        lines.append(f"  Samples with >= 1 flagged species:  {df_flag['sample_id'].nunique()}")
        lines.append(f"  Unique flagged species:             {df_flag['species'].nunique()}")
        lines.append("")
        lines.append("  Top flagged species by prevalence (reads_used = clade or direct):")
        prev = df_flag.groupby("species").agg(
            n_samples=("sample_id","nunique"),
            median_cov=("cov","median"),
            median_reads=("reads_used","median"),
        ).sort_values("n_samples", ascending=False).head(20)
        lines.append(f"  {'Species':<42} {'Samples':>8} {'Med.Cov':>10} {'Med.Reads':>12}")
        lines.append("  " + "-"*74)
        for sp, row in prev.iterrows():
            tag = " [CONTAM]" if any(c.lower() in sp.lower() for c in CONTAMINANTS) else ""
            lines.append(
                f"  {(sp+tag):<42} {int(row['n_samples']):>8} "
                f"{row['median_cov']:>10.5f} {int(row['median_reads']):>12,}"
            )
    lines.append("")
...
def main():
    ap = argparse.ArgumentParser(
        description="HEROIC1K KrakenUniq pipeline — species level, READ-COUNT version"
    )
    ap.add_argument("--report_dir",       required=True)
    ap.add_argument("--metadata",         default=None,
                    help="Path to GEOETHNIC_MASTER_BATCH1.csv")
    ap.add_argument("--output_dir",       default="./heroic1k_results_reads")
    ap.add_argument("--pattern",          default="*.krakenuniq.report")
    ap.add_argument("--cov_threshold",    type=float, default=0.005)
    ap.add_argument("--top_species",      type=int,   default=40)
    ap.add_argument("--use_direct_reads", action="store_true",
                    help="Use reads_direct (assigned to taxon only) instead of "
                         "reads_clade (assigned to taxon + all descendants). "
                         "Default: reads_clade.")
    args = ap.parse_args()
    ...
    print(f"[INFO] Read field:  {'reads_direct' if args.use_direct_reads else 'reads_clade'}")

    # Load reports
    df_summ, df_div, df_flag, df_reads = load_all(
        args.report_dir, args.pattern, args.cov_threshold,
        use_direct_reads=args.use_direct_reads)
    ...
    # Save tables
    read_type = "direct" if args.use_direct_reads else "clade"
    df_merged.to_csv(out_dir / "per_sample_summary.csv", index=False)
    df_div_merged.to_csv(out_dir / "alpha_diversity.csv", index=False)
    df_reads.to_csv(out_dir / f"species_read_matrix_{read_type}.csv")
    if not df_flag.empty:
        df_flag.to_csv(out_dir / "flagged_species.csv", index=False)
        df_flag.pivot_table(index="sample_id", columns="species",
                            values="cov", aggfunc="max", fill_value=0
                            ).to_csv(out_dir / "flagged_cov_pivot.csv")
        df_flag.pivot_table(index="sample_id", columns="species",
                            values="reads_used", aggfunc="max", fill_value=0
                            ).to_csv(out_dir / f"flagged_reads_pivot_{read_type}.csv")
    ...
    plot_read_matrix(df_reads, df_merged, out_dir, top_n=args.top_species)
    plot_flagged_prevalence(df_flag, out_dir, args.cov_threshold)
    plot_isup_diversity(df_div_merged, out_dir)

    write_report(df_merged, df_div_merged, df_flag, df_reads, args.cov_threshold, out_dir)
