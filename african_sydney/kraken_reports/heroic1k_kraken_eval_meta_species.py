#!/usr/bin/env python3
"""
HEROIC1K KrakenUniq Report Evaluation Pipeline — Species Level, with Metadata
==============================================================================
...
#Non depletion usage
python heroic1k_kraken_eval_meta_species.py \
  ...
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_no_depletion/meta_data_analysis_species \
  --cov_threshold 0.005 \
  --top_species 40

#ARKS depletion usage
python heroic1k_kraken_eval_meta_species.py \
  --report_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/arks_mcf_05_depletion_reports \
  --metadata /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/geoethnic_master_batch1.csv \
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_depletion_arks_species \
  --cov_threshold 0.005 \
  --top_species 40
...
python heroic1k_kraken_eval_meta_species.py \
  ...
  --metadata /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/geoethnic_master_batch1.csv \
  --output_dir /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/kraken_reports/heroic1k_results/heroic1k_results_depletion_t2t_species \
  --cov_threshold 0.005 \
  --top_species 40
"""
...
def load_all(report_dir, pattern, cov_threshold):
    ...
    for f in files:
        ...
        kmer_counts = [s["kmers"] for s in species if s["kmers"] > 0]
        diversity.append({
        ...
        for s in species:
            kmer_matrix[s["name"]][sid] = s["kmers"]
            if s["cov"] >= cov_threshold:
                flagged.append({
                    "sample_id": sid, "species": s["name"],
                    "reads_clade": s["reads_clade"], "kmers": s["kmers"],
                    "dup": s["dup"], "cov": s["cov"],
    ...
    df_flag = pd.DataFrame(flagged) if flagged else pd.DataFrame(
        columns=["sample_id","species","reads_clade","kmers","dup","cov","is_contaminant"])
...
def plot_alpha_diversity(df, out_dir):
    ...
    for ax, col, label, color in zip(
        ...
        ["Observed Species", "Shannon Entropy (k-mers)", "Simpson Index (k-mers)"],
...
def plot_kmer_matrix(df_kmer, df_meta_summ, out_dir, top_n=40):
    ...
    ax.set_title(f"Species K-mer Abundance Matrix — Top {top_n} Species (log₁₀, Bray-Curtis clustered)",
                 fontsize=11, fontweight="bold")
...
def write_report(df, df_div, df_flag, df_kmer, cov_threshold, out_dir):
    ...
    lines += [sep, "HEROIC1K KrakenUniq Evaluation — Summary Report (Species Level)", sep, ""]
    lines.append(f"  Samples analysed:         {len(df)}")
    lines.append(f"  Coverage flag threshold:   >= {cov_threshold}")
    lines.append(f"  Total species detected:    {df_kmer.shape[1]}")
    lines.append("")
    ...
    lines.append("── Alpha Diversity (Species Level) " + "─" * 26)
    ...
    if df_flag.empty:
        ...
        lines.append("")
        lines.append("  Top flagged species by prevalence:")
        ...
        lines.append(f"  {'Species':<42} {'Samples':>8} {'Med.Cov':>10} {'Med.Kmers':>12}")
        lines.append("  " + "-"*74)
...
def main():
    ...
    df_kmer.to_csv(out_dir / "species_kmer_matrix.csv")
    ...
    plot_kmer_matrix(df_kmer, df_merged, out_dir, top_n=args.top_species)
