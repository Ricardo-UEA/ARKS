...
#SBATCH -o /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/job_outputs/hdist_0_mcf_05/bbduk_depletion/bbduk%A_%a.out
#SBATCH -e /gpfs/home/hce24xau/scratch/gen_kmers/data/african_sydney/job_errors/hdist_0_mcf_05/bbduk_depletion/bbduk_%A_%a.err
#SBATCH -p compute
#SBATCH --mem=200GB
...
FILE_NAME="hdist_0_mcf_05_GRCh38"
...
#HUMAN_REF="/gpfs/home/hce24xau/scratch/gen_kmers/data/all_kmers/assembled_v2/ref_pans_hla_v2.fasta"
...
bbduk.sh \
    -Xmx200g \
    in1="$R1" in2="$R2" \
    out1="${CLEAN_DIR}/${SAMPLE}_clean_R1.fastq.gz" \
    out2="${CLEAN_DIR}/${SAMPLE}_clean_R2.fastq.gz" \
    outm1="${HUMAN_DIR}/${SAMPLE}_human_R1.fastq.gz" \
    outm2="${HUMAN_DIR}/${SAMPLE}_human_R2.fastq.gz" \
    ref="$HUMAN_REF" \
    k=31 \
    hdist=0 \
    mm=t \
